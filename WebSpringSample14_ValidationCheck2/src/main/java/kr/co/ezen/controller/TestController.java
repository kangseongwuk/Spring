		...
		if(result.hasErrors()) {
			...
			for(ObjectError objectError : result.getAllErrors()) {
				...
				if(codeStrings[0].equals("Size.dataBean2.postcode")) {
					...
					System.out.println("address1는 최소 3자리, 최대 6자리까지 가능."); 
				}else if(codeStrings[0].equals("Size.dataBean2.address2")) {
					System.out.println("address2는 최소 3자리, 최대 30자리까지 가능."); 
				}	
				System.out.println("-----------------------------");	
			}
			return "input_data2";
		}				
		return "input_sucess2";
	}
