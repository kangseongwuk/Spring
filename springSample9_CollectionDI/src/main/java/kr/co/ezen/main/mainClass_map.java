...
public class mainClass_map {
	...
	public static void main(String[] args) {
		...
		System.out.println(map); //{문지영=95, MOON=97, 강성욱=100, kang=90}
		System.out.println(map.size()); // 4
		
		map.remove("kang");
		System.out.println(map.size()); // 3
		System.out.println(map); // {문지영=95, MOON=97, 강성욱=100}
		
		map.put("두루미", 34);
		System.out.println(map.size()); // 4
		System.out.println(map); // {문지영=95, MOON=97, 강성욱=100, 두루미=34} 맨뒤에 들어가는 것이 아닌 메모리가 비어있는 곳으로 들어간다!!
		/* 메모리 사용시
		 * 1) 최초 적합
		 * 2) 최적 적합
		 * 3) 최악 적합
		...
		//객체를 하나씩 찾아서 처리합니다.
		Set<String> keySet = map.keySet(); //맵구조에 접근하는데 keyset을 찾아서 keyset에 저장하겠다.
		//반복저장을 위한 iterator
		Iterator<String> kIterator = keySet.iterator();
		
		while (kIterator.hasNext()) {
			String string = (String) kIterator.next(); // 
			Integer valueInteger = map.get(string);
			
			//4개의 데이터가 반복적으로 나올 것 이다.
			// 95 97 100 34
