...
----------------------------------------------
 ...
 - 주로 많이 사용되는 부분이 로깅, 선언적 트랜잭션, 보안, 캐싱 등 
 		로깅, 트랜잭션, 보안, 캐싱을 사용할 때 aop를 사용한다.
...
----------------------------------------
 ...
 - execution(접근제한자,  반환값(리턴타입), 클래스이름, 메소드명(매개변수(전달인자)) )
   ...
   5) 매개변수 : 매개변수의 형태
	...
	<!--context-aop 설정-->
	<bean id='advisor1' class='kr.co.ezen.advisor.AdvisorClass'/>
	<aop:config>
		<aop:aspect ref='advisor1'>
	<!-- 반환타입은 void, kr.co.korea.beans.TestBean1 클래스, method1 메소드, 매개변수 없음.-->
	<aop:pointcut id="point1" expression="execution(void kr.co.ezen.beans.TestBean1.method1())"/>
	<aop:before method="firstMethod1" pointcut-ref="point1"/>
	<aop:pointcut id="point2" expression="execution(void kr.co.ezen.beans.TestBean1.method2())"/>
	<aop:before method="secondMethod2" pointcut-ref="point2"/>
	<aop:pointcut id="point1" expression="execution(void kr.co.ezen.beans.TestBean1.method1(int))"/>
	<aop:pointcut id="point1" expression="execution(void kr.co.ezen.beans.TestBean1.method1(String))"/>
			
	<aop:pointcut id="sss1" expression="execution(void kr.co.ezen.beans.TestBean1.method1(*))"/>
	<aop:after method="Method3" pointcut-ref="sss1"/>
	
	<aop:pointcut id="aaa1" expression="execution
					(int kr.co.ezen.beans2.TestSample1.add(int x, int y))"/>
   ...
   <!-- 반환타입은 void, kr.co.korea.beans 패키지, TestBean1 클래스, method1 메소드, 모든 타입의 매개변수 0개 이상. -->			
   <aop:pointcut id="point1" expression=
...
<!-- 반환타입은 void, kr.co.korea.beans 패키지, TestBean1 클래스, 모든 메소드, 모든 타입의 매개변수 0개 이상. -->		
	...
	<!-- 반환타입은 void, kr.co.korea.beans 패키지, 모든 클래스, 모든 메소드, 모든 타입의 매개변수 0개 이상. -->				
	<aop:pointcut id="point1" expression=
...
public class mainClass {
	...
	public static void main(String[] args) {
		...
		int sum2 = bean2.method2();
		System.out.println(sum2);
		
		int sum3 = bean2.method3(20);
		System.out.println(sum3); // 400
		
		 bean2.method4(20, "만원");
		
		 bean2.method5(100.123, 20);
		 
		 int sum4 =bean2.method2();
